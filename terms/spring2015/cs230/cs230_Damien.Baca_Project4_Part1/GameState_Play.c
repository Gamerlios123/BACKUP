...
void GameStatePlayUpdate(void)
{
	...
	if (0 == sgStopped || 1 == stopStep)
	{
		...
		for (i = 0; i < PILLARS_NUM / 2; ++i)
		{
			float t = AEReflectAnimatedCircleOnStaticLineSegment(&pBall->posCurr, &newBallPos, BALL_RADIUS, &gPillarsWalls[i], &intersectionPoint, &r);
		...
		if (smallestT > 0.0)
		{
			...
			Vector2DScale(&pBall->velCurr, &r, AEVec2Length(&pBall->velCurr));
		}
...
#if(DRAW_DEBUG)
		{
			...
			velLength = AEVec2Length(&pBall->velCurr);
